        ...
        for h in heights:
            for i in rand_states:
                ...
                for modeltype in modeltypes:
                    ...
                    if b_type == 'ISING':
                        opt_model = MBDT_ising(data=model_set, tree=tree, target=target, modeltype=modeltype,
                    ...
                    if opt_model.model.RunTime < time_limit:
                        print('Optimal solution found in', round(opt_model.model.RunTime,4),'s. ('+ str(time.strftime("%I:%M %p", time.localtime()))+')')
                    else:
