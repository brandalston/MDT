...
# Usage: python3 SOCT_run_MIP.py method max_depth dataset
...
if method == "Full":
    soct = SOCTFull(max_depth=max_depth, ccp_alpha=0, time_limit=time_limit, log_to_console=False)
elif method == "Benders":
    soct = SOCTBenders(max_depth=max_depth, ccp_alpha=0, time_limit=time_limit, log_to_console=False)
...
if soct.branch_rules_ is not None:
    ...
    if method == "Full":
        gap = soct.model_.MIPGap
    elif method == "Benders":
        gap = soct.master_.MIPGap
