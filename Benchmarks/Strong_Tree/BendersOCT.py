'''
This file is the implementation of the BendersOCT model found in the paper ''[Strong Optimal Classification Trees](https://sites.google.com/view/sina-aghaei/home)''.
and publically available on https://github.com/pashew94/StrongTree/Code/StrongTree/BendersOCT.py
Code is taken directly from https://github.com/pashew94/StrongTree/Code/StrongTree/BendersOCT.py
...
class BendersOCT:
    def __init__(self, data, label, tree, _lambda, time_limit, mode):
        ...
        # opt_model.reg_features = None
        # opt_model.num_of_reg_features = 1
    ...
    def create_master_problem(self):
        ...
        For the case regression w[n,1] is the prediction value for node n
        '''
        ...
        # sum(b[n,f], f) + p[n] + sum(p[model], model in A(n)) = 1   forall n in Nodes
        ...
        # opt_model.model.addConstr(
        #     (quicksum(
        #         quicksum(opt_model.b[n, f] for f in opt_model.cat_features) for n in opt_model.tree.Nodes)) <= opt_model.branching_limit)

        # w set
        if self.mode == "classification":  # zeta[i,n] <= w[n,y[i]]     forall n in N+L, i
        ...
        # p[n] + sum(p[model], model in A(n)) = 1   forall n in Leaves
        self.model.addConstrs(
            (self.p[n] + quicksum(
                self.p[m] for m in self.tree.get_ancestors(n)) == 1) for n in
            self.tree.Leaves)
