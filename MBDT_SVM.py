...
class MBDT_SVM:
    ...
    def formulation(self):
        ...
        # CUT1: S[i,v] <= Q[i,c] for c in path to v, for v in V
        # CUT2: S[i,v] + sum(S[i,u] for u in child[v]) <= Q[i,c] for c in path to v, for v in V
    ...
    def optimization(self):
        self.model.optimize(MBDT_SVM.callbacks)
    ...
    def callbacks(model, where):
        ...
        if where == GRB.Callback.MIPSOL:
            ...
            for v in model._tree.B:
                ...
                VIS = MBDT_SVM.VIS(model._data, model._featureset, Lv_I, Rv_I, vis_weight=model._vis_weight)
                if VIS is None:
                    continue
